...
export const DateTimePicker: React.FC<DateTimePickerProps> = ({
  ...
  const formatTriggerLabel = () => {
    if (!isValidEnd && !isValidStart) return null;
    ...
    if (isRangeMode && isValidStart && isValidEnd) {
      return `${formatSingle(startD)} ~ ${formatSingle(endD)}`;
    }

    return isValidEnd ? formatSingle(endD) : isValidStart ? formatSingle(startD) : null;
  };
  ...
  return (
    <div className={`relative inline-block w-full ${className}`}>
        ...
        <div className="flex items-center gap-2 min-w-0 flex-1">
          ...
          {isValidEnd || isValidStart ? (
            <div className="flex items-center gap-2 flex-wrap min-w-0">
              <span className="text-xs font-bold leading-relaxed">{formatTriggerLabel()}</span>
              {dueDateStatus && !isRangeMode && (
                <span
      ...
      {/* Floating Popover Picker - Solid high-contrast background */}
      {isOpen && (
        <>
          ...
          {/* Popover Container - 100% Solid background with deep shadow */}
          <div
            className={`absolute top-full mt-2 z-50 w-80 sm:w-92 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-3xl shadow-2xl p-4 animate-in fade-in zoom-in-95 duration-150 space-y-3.5 text-slate-800 dark:text-slate-100 ${
