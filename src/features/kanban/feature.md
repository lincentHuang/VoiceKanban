...
- **Success**：任務標記完成、批次操作、欄位新增、重排、展開為欄位或聚合為任務卡片完成時給予彩帶特效（Confetti）或震動反饋。
...
- [x] **AC-KANBAN-8**：多選批次操作列功能（批次移動/刪除/改優先級）正常運作，且手機端（`< sm`）以雙行流暢佈局呈現，文字不跑版擠壓，桌機端保持單行膠囊列。
...
- [x] **AC-KANBAN-16**：**安全任務刪除防呆與全平台確認 Modal 一致性 (Safe Task Deletion & Unified Confirmation Modal)**：
  ...
  - 任務詳細視窗以任務 ID 為 key 重新掛載，刪除確認、封面選擇、移動與展開確認等子視窗狀態不會殘留至下一張卡片（修正刪除後開啟其他任務又跳出刪除確認）；點擊刪除確認外圍僅取消刪除、不連帶關閉任務視窗，預設焦點落在「取消」。
...
- [x] **AC-KANBAN-19**：**手機版欄位拖曳重排 (Mobile Column Drag Reorder)**：
  - 手機上長按欄位標頭 200ms 即可拖曳整個欄位改變順序，拖到畫面邊緣會自動翻到隔壁欄。
  - 欄位拖曳的碰撞判定只比對「欄位」，並以手指的水平位置決定落點；修正先前對所有 droppable 做 `closestCenter`，手機上常判定到隔壁欄的卡片，導致放開後順序不變的問題。
- [x] **AC-KANBAN-20**：**欄位移動／複製到其他看板 (Move / Copy Column to Another Board)**：
  - 欄位選單（桌機下拉子選單「移動列表到其他看板」「複製列表到其他看板」、手機底部面板「移動或複製到其他看板」）可把整個欄位連同卡片移到或複製到其他看板。
  - **共享（群組）看板的欄位只能複製出去，不能移動出去**；個人看板的欄位可以移動或複製進共享看板。看板只剩一個欄位時也只能複製。
  - 只列出目前使用者有編輯權限的目標看板（檢視者角色的共享看板不會出現）。
