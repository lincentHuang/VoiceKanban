...
- [x] **AC-KANBAN-19**：**手機版不提供欄位拖曳換位 (No Column Drag on Mobile)**：
  - 手機（`< 640px`）上欄位標頭不顯示 `⋮⋮` 拖曳把手，長按也不會拖起整個欄位；欄位順序只在「看板管理 → 欄位流程」裡調整。卡片仍可拖進欄位。
  - 桌機維持標頭拖曳重排；欄位拖曳的碰撞判定只比對「欄位」並以游標水平位置決定落點。
