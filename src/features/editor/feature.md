...
2. **快捷格式工具列 (Formatting Toolbar)**：
   - 提供粗體（B）、斜體（I）、標題（H1/H2）、代碼塊（Code）、引用（Quote）、清單（List）等一鍵插入工具。
3. **語法高亮與精緻排版 (Typography & Highlighting)**：
   - 採用 GitHub Flavored Markdown 樣式，清晰渲染程式碼區塊、待辦方塊與表格。
...
src/features/editor/
├── components/
│   └── MarkdownEditor.tsx     # Markdown 雙模編輯器元件 (含工具列與預覽切換)
├── index.ts                   # 模組統一出口
...
- **Previewing**：預覽模式下唯讀渲染精美 HTML排版。
- **Loading**：大型筆記解析時無卡頓渲染。
- **Error**：非標準字元或解析異常時降級為純文字安全顯示。
