# 測試驗收報告 (QA_REPORT.md) - 多人即時協同編輯與邀請機制 (Collaborative Multi-user Kanban & Real-Time Sync)

## 1. 測試摘要 (Executive Summary)
- **測試目標**：
  1. **看板專屬 6 碼短代碼與一鍵邀請連結 (Invite Code & Share Link)**：建立者於頂部工具列點擊「邀請協作」時，自動生成專屬 6 碼代碼（如 `VK-8X4B`）與完整邀請連結，支援一鍵複製與 Toast 提示。
  2. **極速加入看板 (Join Board Flow)**：在看板切換選單或頂部 Navbar 點擊「加入協作看板...」開啟 `JoinBoardModal` 輸入代碼加入；若網址帶有 `?invite=CODE` 查詢參數，系統自動啟動並預填代碼。
  3. **未登入訪客 (Guest) 免註冊極速協同**：未登入訪客無需經歷繁瑣註冊流程，輸入「協作者暱稱」即可由系統自動指派頭像秒級加入，日後登入時無縫關聯歷史紀錄。
  4. **三級角色權限模型 (Owner / Editor / Viewer)**：Owner 具備最高管理權，可升降成員權限（Editor ↔ Viewer）或移出成員；Viewer（檢視者）進入唯讀保護模式，卡片與欄位拖曳被安全停用，隱藏新增卡片按鈕，並於頂部常設「👁️ 唯讀模式」溫和提示條。
  5. **雙軌即時同步引擎 (Firestore onSnapshot + BroadcastChannel)**：線上環境透過 Firestore `shared_boards` 集合雙向監聽；本機/跨分頁環境透過 `BroadcastChannel` 實現 0 延遲即時同步，任一分頁拖曳或修改卡片，其他分頁即刻連動更新。
  6. **頂部協作者頭像堆疊 (Collaborator Avatars)**：看板頂部即時呈現所有協同成員頭像堆疊、名稱與角色徽章（👑 Owner、✏️ Editor、👁️ Viewer），提供直覺視覺反饋。
- **測試結果**：全部 5 項功能驗收標準 (AC-COL-1 ~ AC-COL-5) 與 8 項單元/整合測試 **100% 通過 (PASS)**。
- **建置狀態**：`npm run build` Next.js 16.3.3 (Turbopack) 生產構建與 TypeScript 型別檢查 0 錯誤通過，Dev Server 於 `http://localhost:3011` 穩定運行中。
...
| :--- | :--- | :--- | :--- | :--- |
| **AC-COL-1** | **邀請代碼與分享連結生成** | 建立者點擊頂部「邀請協作」，開啟 `ShareBoardModal`。 | 系統自動為看板生成合法 6 碼大寫代碼（`VK-XXXX`）與專屬網址，點擊「複製代碼」或「複製連結」即刻反饋「✓ 已複製」，字串成功寫入剪貼簿。 | ✅ PASS |
| **AC-COL-2** | **代碼輸入與 URL 自動帶入加入流程** | 1. 於選單開啟 `JoinBoardModal` 輸入代碼；2. 開啟帶有 `?invite=VK-XXXX` 網址。 | 1. 輸入合法代碼成功加入並切換至該看板；2. 帶參網址自動開啟加入視窗且預填代碼，點擊即完成加入。 | ✅ PASS |
| **AC-COL-3** | **訪客免登入暱稱極速協同** | 訪客身分在未登入狀態下加入協作看板，輸入自訂暱稱「設計師 Alex」。 | 成功以匿名協作者身分加入，系統為其指派 DiceBear 頭像，看板頂部正確顯示暱稱與編輯者權限。 | ✅ PASS |
| **AC-COL-4** | **三級角色權限與唯讀保護** | 1. Owner 於管理彈窗將成員切換為「檢視者」；2. 檢視者嘗試拖曳或新增卡片。 | 1. 成員角色即刻更新為 `viewer`；2. 檢視者畫面自動呈現「👁️ 唯讀模式」提示條，卡片拖曳與欄位拖曳安全禁用，底端新增按鈕自動隱藏。 | ✅ PASS |
| **AC-COL-5** | **雙軌即時同步與頭像堆疊** | 在同瀏覽器開啟多個分頁或跨裝置同時檢視同一個協作看板。 | 分頁 A 拖曳任務卡片或新增內容，分頁 B 透過 `BroadcastChannel` 與 Firestore `onSnapshot` 秒級無縫更新，頂部頭像堆疊即時列出所有成員。 | ✅ PASS |
...
1. **Loading 狀態**：
   - 點擊「邀請協作」尚未生成金鑰時，呈現微光 Spinner 與「正在為此看板生成專屬協同金鑰與邀請連結...」提示。
   - 點擊「加入協作看板」時按鈕呈現旋轉動畫與 Disabled 禁用態，防範連續重複點擊。
2. **Empty 狀態**：
   - 看板僅有建立者 1 人時，呈現「目前尚無其他協作者」虛線引導卡片，提示分享代碼邀請好友。
3. **Error 狀態**：
   - 輸入不存在或已失效之代碼時，彈出鮮明紅色警告：「查無此邀請代碼，請確認代碼是否輸入正確或已失效」。
4. **Success 狀態**：
   - 複製代碼/連結成功切換為翠綠色 Checkmark 圖示與「已複製」文字。
   - 成功加入看板時顯示綠色 CheckCircle2 橫幅與微動畫，自動導航進入目標看板。
5. **Active 狀態**：
   - 協作者頭像堆疊具備微光邊框與 Hover Tooltip，顯示成員姓名與角色（👑/✏️/👁️）。
   - 看板切換選單標註「👥 協作」橘色精緻徽章。
