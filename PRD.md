...
- **固定側邊欄與滑順展開收合動畫 (Animated Fixed Sidebar Inbox)**：
  ...
  - **手機版抽屜滑入滑出動畫 (Mobile Slide-in/out)**：手機版以 300ms `ease-in-out` 由左側邊緣平滑滑入（Slide-in）與滑出（Slide-out），支援手勢與按鈕切換。
...
- [x] **MAC-23 (多入口任務即時刪除與雲端徹底抹除)**：
  - 看板卡片與橫列視圖（`TaskCard`）提供懸停/觸控一鍵快速刪除按鈕。
  - 任務詳細視窗（`EditTaskModal`）頂部操作列常設刪除垃圾桶圖示與防呆確認 Popover，底部同步提供確認刪除區塊。
